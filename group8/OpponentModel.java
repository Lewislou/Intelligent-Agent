...
public class OpponentModel {
    ...
    */
    ...
    public void updateModel(UtilitySpace utilitySpace) {
        ...
        for (Issue issue : utilitySpace.getDomain().getIssues()) {
            ...
            for(int j=0;j<k;j++){
                ...
                //System.out.println("The w1 value is"+w1);
                if(w1<0){
                    System.out.println("The w1 value is"+w1);
                }
                w += w1;
            ...
            //System.out.println("The weight of issue"+ w);
    ...
    public double getUtility(Bid bid) {
        ...
        return (utility/bid.getIssues().size());
    }
    private double penalty() {
        double time = timeline.getCurrentTime()/ timeline.getTotalTime();
        //System.out.println("The time value is"+time);
        //double remainingTimeRatio = timeline.getCurrentTime() / timeline.getTotalTime();
        //double penalty = Math.pow(remainingTimeRatio, Math.E);
        double penalty = Math.pow(0.5,time);
        return penalty;
    }
